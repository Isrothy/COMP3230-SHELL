...
isr_hash_table_new(size_t (*hash_func)(void *), int (*id_func)(void *, void *)) {
    struct ISRHashTable *table = (struct ISRHashTable *) malloc(sizeof(struct ISRHashTable));
    table->hash_func = hash_func;
    table->id_func = id_func;
    table->capacity = 7;
    table->size = 0;
    table->lists
        = (struct ISRLinkedList **) malloc(table->capacity * sizeof(struct ISRLinkedList *));
...
int isr_hash_table_remove(struct ISRHashTable *table, void *key, void *value) {
    size_t hash_value = table->hash_func(key);
    size_t index = hash_value % table->capacity;
    struct ISRLinkedList *l = table->lists[index];
    for (struct ISRLinkedListNode *p = l->sentinal; p->next != NULL; p = p->next) {
        struct ISRLinkedListNode *q = p->next;
        struct ISRHashTableEntity *o = (struct ISRHashTableEntity *) q->value;
        if (table->id_func(o->key, key)) {
            p->next = q->next;
            free(q->value);
            free(q);
